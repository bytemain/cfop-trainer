...
describe("relative body rotation", () => {
  it("derives the local-axis 90 degree turn from a GAN16 validation capture", () => {
    const rotation = relativeBodyRotation(
    ...
    expect(rotation?.axis.x).toBeCloseTo(-0.78555, 3);
    expect(rotation?.axis.y).toBeCloseTo(-0.61132, 3);
    expect(rotation?.axis.z).toBeCloseTo(0.09586, 3);
  });

  it("recognizes opposite directions on the same learned body axis", () => {
    const axis = { x: 0.8, y: -0.6, z: 0 };
    const positiveReference = positiveAxisReference(axis, "positive");
