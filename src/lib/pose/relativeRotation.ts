...
export function relativeProtocolRotation(
  ...
  // GAN V4 reports a passive world -> cube-body quaternion, so the runtime
  // SSOT relative rotation is reference * inverse(current). Keeping
  // validation in the same order makes its sensor-axis evidence directly
  // comparable to GAN_V4_SENSOR_AXES instead of learning a per-user
  // compensation.
  let delta = {
