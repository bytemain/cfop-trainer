<script lang="ts">
  ...
  const colorVectors: Record<CubeColor, [number, number, number]> = {
    white: [0, 1, 0],
    yellow: [0, -1, 0],
    green: [0, 0, 1],
    blue: [0, 0, -1],
  ...
  function colorForVector(vector: [number, number, number]): CubeColor {
    return (Object.entries(colorVectors) as Array<[CubeColor, [number, number, number]]>)
      .find(([, candidate]) => candidate.every((value, index) => value === vector[index]))?.[0] ?? "red";
  }

  function rightColor(topColor: CubeColor, frontColor: CubeColor): CubeColor {
    const [tx, ty, tz] = colorVectors[topColor];
    const [fx, fy, fz] = colorVectors[frontColor];
    return colorForVector([
      ty * fz - tz * fy,
      tz * fx - tx * fz,
      tx * fy - ty * fx,
    ]);
...
<div class="guide-card-3d" class:dynamic={mode === "dynamic"}>
  <div class="guide-scene" aria-hidden="true">
    ...
    {:else}
      <svg class="rotation-arrow" viewBox="0 0 200 200" role="img" aria-label="顺时针转动整颗魔方约九十度">
        <defs>
