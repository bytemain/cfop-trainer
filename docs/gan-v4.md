...
传感器世界系跨会话基本可复现（流数据实测 15 天间隔漂移约 9°），因此没有会话锚点时用 `GAN_V4_IDENTITY_SENSOR_POSE`（白上绿前标准握持的真机 fixture 读数，与 `orientation.test.ts` 同源）充当 reference，首帧起即近似绝对跟踪；快速校准换成当前会话自己的白上绿前读数后残差归零。注意 reference 必须是标准握持读数，绝不能用连接时的握持——流数据显示后者会把每个转动共轭一遍任意连接姿态（实测 77° 轴误差）。显示姿态按 `Rdisplayed = Rreference · Rdelta` 合成——增量右乘参考姿态；左乘会把每个转动共轭一遍参考姿态。注意 180° fixture 无法区分乘法顺序，方向/轴向证据必须来自受控的非 180° 转动或流数据回放。

应用连接 GAN V4 时必须覆盖历史本地 axis calibration，不能要求普通用户重复三轴采集。首个姿态帧经固定契约转换成近似绝对 `CubePose` 后建立 session anchor；anchor 负责掉线/传感器重启后的连续性，快速校准负责白上绿前的语义绑定（消除跨会话残差）。
